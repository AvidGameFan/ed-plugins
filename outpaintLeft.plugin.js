...
const outpaintLeftSizeIncrease = 128;
...
PLUGINS['IMAGE_INFO_BUTTONS'].push({
  ...
  on_click: function(origRequest, image) {
    ...
    var imageData = ctx.getImageData(0, Math.max(canvas.width,outpaintLeftSizeIncrease), outpaintLeftSizeIncrease, canvas.height);
...
// Only for cropped-source:      0, 0,origRequest.width, origRequest.height-origRequest.height/4, //source crop
      ...
      outpaintLeftSizeIncrease, 0, origRequest.width, origRequest.height //destination
...
//    Now reassemble old and new
  ...
  result = false
//  var ratio=Math.sqrt(maxTotalResolution/(origRequest.height*origRequest.width));
  if ((origRequest.width+outpaintLeftSizeIncrease)*origRequest.height<=maxTotalResolution)  {
