    /**
     * Scale Up BIG
     * v.1.09, last updated: 11/24/2022
     * By Gary W.
     * 
     * Large scaling up in size, to the maximum resolution of your video card, while maintaining 
    ...
    var maxTotalResolution = 1536 * 960;  //put max resolution (without turbo mode) here
    var maxTurboResolution = 1280 * 1024;  //put max turbo resolution here
        ...
        on_click: function(origRequest, image) {
            function scaleUp(height,width) {
                ...
                if (result==height /*no match found in table*/
                    ...
                    {
                      result=MaxSquareResolution;
                    }
                return result;
              }
        
            //Find the biggest
            var workingHeight=origRequest.height;
            var workingWidth=origRequest.width;
            do {
              var tempWidth=scaleUp(workingWidth, workingHeight);
              var tempHeight=scaleUp(workingHeight, workingWidth);
              if (tempWidth==workingWidth && tempHeight==workingHeight)  //if no further changes, exit
                break;
              if (tempWidth*tempHeight<=maxTotalResolution)
              {
                workingWidth=Math.max(tempWidth, workingWidth);
                workingHeight=Math.max(tempHeight, workingHeight);
              }
            } while (true) 
